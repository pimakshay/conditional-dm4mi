...
from diffusion_modules.diffusion_utils.beta_schedule import make_ddim_timesteps, make_ddim_sampling_parameters, noise_like


class DDIM (pl.LightningModule):
    def __init__(self, ddpm_model: DDPM, schedule='linear', **kwargs) -> None:
        super().__init__()
        self.ddpm_model = ddpm_model
        self.ddpm_timesteps = ddpm_model.num_timesteps
    ...
    def make_schedule(self, ddim_timesteps, ddim_discretize="uniform", ddim_eta=0.0, verbose=True):
        self.ddim_timesteps = make_ddim_timesteps(
            ddim_discr_method=ddim_discretize, num_ddim_timesteps=ddim_timesteps, num_ddpm_timesteps=self.ddpm_timesteps)
        alphas_cumprod = self.ddpm_model.alphas_cumprod
        assert alphas_cumprod.shape[0] == self.ddpm_timesteps, 'alphas have to be defined for each timestep'

        # def to_torch(x): return x.clone().detach().to(
        #     torch.float32).to(self.ddpm_model.device)
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_timesteps=S, ddim_eta=eta, verbose=verbose)
        # sampling
        C, H, W = shape
        ...
        samples, intermediates = self.ddim_sampling(#conditioning, 
                                                    ...
                                                    mask=mask, x0=x0,
                                                    ddim_use_original_steps=use_ddpm_sampling,
    ...
    def ddim_sampling(self, 
        ...
        if timesteps is None:
            timesteps = self.ddpm_timesteps if ddim_use_original_steps else self.ddim_timesteps
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None:
                ...
                img_orig = self.ddpm_model.q_sample(x0, ts)
    ...
    def p_sample_ddim(self, 
                      x, 
                    #   c, 
                      t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., 
                    #   score_corrector=None, corrector_kwargs=None,
                    #   unconditional_guidance_scale=1., unconditional_conditioning=None
                      ):
        b, *_, device = *x.shape, x.device
        e_t = self.ddpm_model.model(x, t,)
        # if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
        #     e_t = self.ddpm_model(x, t,) #.apply_model(x, t, c)
        # else:
        #     x_in = torch.cat([x] * 2)
        #     t_in = torch.cat([t] * 2)
        #     c_in = torch.cat([unconditional_conditioning, c])
        #     e_t_uncond, e_t = self.ddpm_model(x_in, t_in) #.apply_model(x_in, t_in, c_in).chunk(2)
        #     e_t = e_t_uncond + unconditional_guidance_scale * \
        #         (e_t - e_t_uncond)

        # if score_corrector is not None:
        #     assert self.ddpm_model.parameterization == "eps"
        ...
        alphas_prev = self.ddpm_model.alphas_cumprod_prev if use_original_steps else self.ddim_alphas_prev
        sqrt_one_minus_alphas = self.ddpm_model.sqrt_one_minus_alphas_cumprod if use_original_steps else self.ddim_sqrt_one_minus_alphas
